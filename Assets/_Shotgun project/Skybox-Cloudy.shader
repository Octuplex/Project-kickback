...
Properties {
    ...
    _SkyboxDistortion("Planet surf, cloudlayer, radius, bumpy", Vector) = (100.0, 60.0, 1000.0, 5.0)
    // speed was 0.03 , tint color was 0.5
    _ScaleSpeedTint("Scale, speed, tint color, bumpscale", Vector) = (0.1, 0.1, 0.68, 0.2)
    ...
    _NoiseTexture ("Noise Texture", 2D) = "white" {}
	[IntRange] _UseNoiseTexture("Use Noise Texture (required on Quest)", Range(0,1)) = 1.0
}
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        if (distToPlane < _CloudFarClip.y && distToPlane > -_CloudFarClip.x) {
            ...
            if (_UseNoiseTexture > 0.5) {
                currentPlaneDist = max(0.2, cloudSurface + tex2D(_NoiseTexture, uv * bumpscale).r * cloudBumpiness + _WorldSpaceCameraPos.y);
            } else {
            ...
            float f, c, q;
            if (_UseNoiseTexture > 0.5) {
                float4 textmp = tex2D(_NoiseTexture, 0.1 * uv * cloudscale + time) + tex2D(_NoiseTexture, 0.1 * float2(0.4, 0.51) * uv * cloudscale + 0.3 * time);
                c = sqrt(dot(textmp.rgb, textmp.rgb) / 12);
                f = 0;
            } else {
            ...
            col = float4(lerp(col, skytint * col + cloudcolour * ndotl, saturate(f + c)), 1.0);;
        }
		#endif
